...
export default class TestToggler {
    ...
    createTestCode(relPath) {
        ...
        return atom.config.get('js-test-toggler.testTemplate').replace(/<(.*?)>/g, (match, key) => {
    ...
    codePath() {
        return atom.config.get('js-test-toggler.codePath');
    }

    testPath() {
        return atom.config.get('js-test-toggler.testPath');
    }
}
